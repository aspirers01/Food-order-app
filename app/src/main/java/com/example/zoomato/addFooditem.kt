        ...
        binding.uploadBtn.setOnClickListener{
           // getting the values from views
            ...
            if (foodname.isBlank() || foodprice.isBlank() || fooddesc.isBlank() || fooding.isBlank() || foodimage==null){
                ...
                if (foodimage==null)
                    Toast.makeText(this, "please fill all data and select image ", Toast.LENGTH_SHORT).show()
            }else{
            ...
            storage.putFile(foodimage!!).addOnCompleteListener{
                if (it.isSuccessful){
                    storage.downloadUrl.addOnSuccessListener {
                       val newitem= MenuModel(foodname,foodprice,it.toString(),fooddesc,fooding)
                        ref.child(id!!).setValue(newitem)
                    }
                }else{
                    Toast.makeText(this, "upload is failed pleasse try again ", Toast.LENGTH_SHORT).show()
                }
            }
         }
